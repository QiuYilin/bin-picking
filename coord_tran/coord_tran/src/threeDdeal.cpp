...
//pcl::visualization::CloudViewer viewer("Cloud in box after removing plane");
   
   std::vector<float> CoordinateTran::threeDdeal (pcl::PointCloud<pcl::PointXYZ>::Ptr cloud){

     
   //pcl::visualization::PCLVisualizer viewer("Cloud");//创建一个可视化窗口
   
   pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_f (new pcl::PointCloud<pcl::PointXYZ>), cloud_final (new pcl::PointCloud<pcl::PointXYZ>);
   pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_filtered (new pcl::PointCloud<pcl::PointXYZ>);
   std::vector<float> result(6);//x y z r h w
   //清无效点
    cloud->is_dense = 0;
   ...
   //viewer1.showCloud(cloud);
    ...
    Static.setStddevMulThresh(1.5);//比例系数α  判断是否为离群点的阈值
  ...
  seg.setDistanceThreshold (0.01);//距离阈值，点被认为是局内点必须满足的条件  多厚的面
  ...
  tree->setInputCloud (cloud_filtered);
   ...
   ec.setClusterTolerance (0.01); // 单位是m
...
//     std::cout << "cloud_cluster_" << j<< " is saved." << std::endl;
    j++;
  }
//    std::cout << "cloud_cluster is over." << std::endl;
  // 在这些点中最小的聚类为需要的
...
//    std::cout << "min element is " << *smallest<< " at position " << min_position<< std::endl;
  ...
  //最小包围盒
   pcl::PCDReader reader;
    ...
    ss2 << "cloud_cluster_" << min_position << ".pcd";
...
//     5) given a box centered at the origin with size (max_pt.x - min_pt.x, max_pt.y - min_pt.y, max_pt.z - min_pt.z) the transformation you have to apply is Rotation = (e0, e1, e0 X e1) & Translation = Rotation * center_diag + (c0, c1, c2)
  // start calculating time
    ...
    result[0] = pcaCentroid[0];
    result[1] = pcaCentroid[1];

    
    float x1 = eigenVectorsPCA(0,2) ;
    float y1 = eigenVectorsPCA(1,2);
    //将二、三象限的向量换个方向
    result[3]=asin(y1/sqrt(x1*x1+y1*y1))* 180 / 3.14;//相对机械臂坐标系为负
    std::cout << "rsult[3]" << result[3]  << std::endl;
...
//     viewer.addPointCloud(cloud_final, color_handler, "cloud_final");//添加点云
//     viewer.addCube(bboxT, bboxQ, whd(0), whd(1), whd(2), "bbox");//平移矩阵 单位四元数 长 宽 高
